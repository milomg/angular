...
export class DirectiveExplorerComponent {
  ...
  watchSignal(a: { node: PropertyFlatNode, directivePosition: DirectivePosition }) {
    let parentPath: string[] = [];
    let current = a.node.prop;
    while (current) {
    ...
    this.signalsTab().addSignal({
      nodePath: propPath,
      directivePosition: a.directivePosition
    });
  }
}
